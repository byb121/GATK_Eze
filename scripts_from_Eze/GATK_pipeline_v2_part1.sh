...
# execute using: 
# bsub -J "GATKp1[1]" < scripts/GATK_pipeline_v2_part1.sh
...
# 	else
# 		echo "$(date '+%d/%m/%y_%H:%M:%S'), ERROR: Picard MarkDuplicates on /data/sutst/DMP/CELLTRAN/NGS/prostate_ICGC/Data/Sample_BAMs/$sample.bam not completed. ExitValue = $exitValue" >> "$samplelog"
# 		exit $exitValue
# 	fi	
...
#java -jar /apps/picard-tools/2.0.1/picard.jar ValidateSamFile I=sample1.bam MODE=SUMMARY
...
if [ ! -f $root_folder/logs/part_1_Picard_ValidateSamFile_finished_$sample.txt ]; then
	...
	# else
	# 	echo "$(date '+%d/%m/%y_%H:%M:%S'), ERROR: Picard ValidateSamFile not completed. ExitValue = $exitValue" >> "$samplelog"
	# 	exit $exitValue
	# fi
...
# python report_metrics_excel.py
# rsync -aWv cyberman:/scratch/cancgene/eanokian/ICGC/metrics_summary.xlsx .
